...
// Only for Ridge Regression & Lasso
std::vector<double>* grad_desc_dense::SVRG_SD(double* X, double* Y, size_t N, blackbox* model
    ...
    , double* SV, bool is_store_result) {
    // Random Generator
    std::random_device rd;
    std::default_random_engine generator(rd());
    std::uniform_int_distribution<int> distribution(0, N - 1);
            ...
            if(!((j + 1) % interval)) {
                ...
                switch(regular) {
                    ...
                    case regularizer::L1: {
                        ...
                        double denominator = SVx / N + zeta * square_p;
...
std::vector<double>* grad_desc_dense::SAGA_SD(double* X, double* Y, size_t N, blackbox* model
    , size_t iteration_no, size_t interval, double L, double sigma, double step_size, double r
    , double* SV, bool is_store_result) {
    // Random Generator
    std::random_device rd;
    ...
    for(size_t i = 0 ; i < iteration_no; i ++) {
        ...
        for(size_t j = 0; j < inner_m; j ++) {
            ...
            for(size_t k = 0; k < MAX_DIM; k ++) {
                double val = X[rand_samp * MAX_DIM + k];
                // Update Weight
                double vr_sub_grad = (inner_core - past_grad_core) * val + aver_grad[k];
                y[k] = x[k] - step_size * vr_sub_grad;
                regularizer::proximal_operator(regular, y[k], step_size, lambda);
                // Update Gradient Table Average
