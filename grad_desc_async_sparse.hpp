...
namespace grad_desc_async_sparse {
    ...
    std::vector<double>* A_Katyusha_Async(double* X, double* Y, size_t* Jc, size_t* Ir, size_t N, blackbox* model, size_t iteration_no
        , size_t thread_no, double L = 1.0, double sigma = 0.0001, double step_size = 1.0, bool is_store_result = false);
    void A_Katyusha_Async_Inner_Loop(double* X, double* Y, size_t* Jc, size_t* Ir, size_t N
        , std::atomic<double>* x, std::atomic<double>* y, std::atomic<double>* z, std::atomic<double>* aver_y, blackbox* model
        , size_t m, size_t inner_iters, double L, double sigma, double step_size, double tau_1, double tau_2, double alpha
