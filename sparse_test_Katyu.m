...
fprintf('Algorithm: %s\n', algorithm);
tic;
hist4 = Interface(X, y, algorithm, model, regularizer, init_weight, lambda1, L, step_size, loop, is_sparse, Mode, sigma, lambda2, thread_no);
time = toc;
fprintf('Time: %f seconds \n', time);
hist4 = [X_SVRG, hist4];
clear X_SVRG;
...
% fprintf('Algorithm: %s\n', algorithm);
% tic;
% hist5 = Interface(full(X), y, algorithm, model, regularizer, init_weight, lambda1, L, step_size, loop, 0, Mode, sigma, lambda2);
% time = toc;
% fprintf('Time: %f seconds \n', time);
% X_SVRG = [0:3:passes]';
% hist5 = [X_SVRG, hist5];
...
%     aa5 = min(hist5(:, 2));
    % aa6 = min(hist6(:, 2));
    minval = min([aa1, aa2, aa3, aa4]) - 2e-16;
    aa = max(max([hist1(:, 2)])) - minval;
    b = 1;

    figure(101);
    set(gcf,'position',[200,100,386,269]);
    semilogy(hist1(1:b:end,1), abs(hist1(1:b:end,2) - minval),'k--o','linewidth',1.6,'markersize',4.5);
    hold on,semilogy(hist2(1:b:end,1), abs(hist2(1:b:end,2) - minval),'r-.^','linewidth',1.6,'markersize',4.5);
    hold on,semilogy(hist3(1:b:end,1), abs(hist3(1:b:end,2) - minval),'b--+','linewidth',1.2,'markersize',4.5);
    hold on,semilogy(hist4(1:b:end,1), abs(hist4(1:b:end,2) - minval),'m-.d','linewidth',1.2,'markersize',4.5);
%     hold on,semilogy(hist5(1:b:end,1), abs(hist5(1:b:end,2) - minval),'g-.^','linewidth',1.2,'markersize',4.5);
    ...
    legend('AsyAcc1', 'AsyAcc2', 'ASAGA', 'Prox-ASVRG');
end